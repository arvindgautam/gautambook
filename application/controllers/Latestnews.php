...
class Latestnews extends CI_Controller {
	...
	public function index()
	{
		 if($this->session->userdata('logged_in'))
		{
			...
			$this->load->view('admin/left_sidebar',$data);
				$updated = $this->input->post('news');
				if(!empty($updated))
				{
					$post_msg = $this->comman_function->add_news();
					
					$post_data['post_msg'] = $post_msg;
					$this->load->view('admin/latest_news',$post_data);
				}
				else
				{
				...
				$updateds = $this->input->post('news_id');
				if(!empty($updateds))
				{
					$post_msgs = $this->comman_function->edit_news();
					redirect('latestnews', 'refresh');
	...
	
	{
		if (isset($_POST['new_id'])) 
		{
			...
			$results = $this->comman_function->delete_latest_news($_POST['new_id']);
