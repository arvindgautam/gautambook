...
class Addstate extends CI_Controller {
	...
	public function index()
	{
		 if($this->session->userdata('logged_in'))
		{
			...
			$this->load->view('admin/left_sidebar',$data);
				...
				if(!empty($updated))
				{
					$post_msg = $this->comman_function->add_state();
					$post_data['post_msg'] = $post_msg;
					$this->load->view('admin/add_state',$post_data);
				}
				else
				{
					$post_msgs = $this->comman_function->edit_state();
					$post_data['post_msg'] = '';
					$this->load->view('admin/add_state',$post_data);
				}
	...
	
	{
		if (isset($_POST['state_id'])) 
		{
			...
			$results = $this->comman_function->delete_states($_POST['state_id']);			// Now we verify the result
			if($results){
